		...
		if (goal.goalState==1):
			...
			while (not rospy.is_shutdown()) and (goalReached == 0):
				...
				if (edgeCounter == 2):
					...
					if (blockCenterAngle > 0) and (abs(blockCenterAngle) > np.pi/100):
						idealTurn.angular.z = 0.05
						rospy.loginfo("turn left")
					elif (blockCenterAngle < 0) and (abs(blockCenterAngle) > np.pi/100):
						idealTurn.angular.z = -0.05
		...
		# get close enough to the block
			...
			while (not rospy.is_shutdown()) and (goalReached == 0):
				...
				if (edgeCounter == 2):
					...
					if (scan.ranges[blockCenteri] > 0.13):
						cmdvel.linear.x = 0.03
					elif (scan.ranges[blockCenteri] < 0.08):
						cmdvel.linear.x = -0.03
					else:
						cmdvel.linear.x=0.0
						goalReached = 1
